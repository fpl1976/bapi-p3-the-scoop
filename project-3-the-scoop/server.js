...
function createComment(url, request) {
  ...
  if (requestComment &&
    ...
    userExists(requestComment.username) &&
    articleExists(requestComment.articleId)) {
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    // For bad requests return 400
    response.status = 400;
  } else if (!savedComment) {
    // For non-existing comments return 404
    response.status = 404;
  } else {
    // Update comment (body)
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    // Delete commment ID from article's comment IDs
    ...
    // Delete comment ID from user's comment IDs
...
function upvoteComment(url, request) {
  ...
  // For invalid requests return 400
  if (!id || !username || !savedComment || !userExists(username)) {
    response.status = 400;
  } else {
    // Upvote comment
...
function downvoteComment(url, request) {
  ...
  // For invalid requests return 400
  if (!id || !requestUsername || !savedComment || !userExists(requestUsername)) {
    response.status = 400;
  } else {
    // Downvote comment
